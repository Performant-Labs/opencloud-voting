...
| **Dark Mode Theming (Inherited)** | Extension contains zero dark mode code. All component styles reference `var(--oc-color-*, fallback)` CSS variables. OpenCloud's `useThemeStore` Pinia store applies themes by calling `document.querySelector(":root").style.setProperty("--oc-" + prefix, value)` at runtime, redefining `--oc-role-*` and `--oc-color-*` tokens directly on `:root`. Each theme carries an `isDark` boolean and a `designTokens` object with `roles` and `colorPalette` maps. | Extensions must never define their own dark mode overrides (no `.dark` CSS classes, no `useDark` composables, no `prefers-color-scheme` queries). The host shell's Pinia store is the single source of truth for theming. Since CSS custom properties cascade, any `var(--oc-color-*)` reference in extension components automatically inherits the active theme's values with zero additional code. |
