...
(async () => {
  ...
  const alphaSpace = alphaDrives.value.find(d => d.driveType === 'project' && d.name.startsWith('Feature Voting Data'));
  if (!alphaSpace) {
  ...
  const putRes = await alphaContext.put(davUrl, {
    data: JSON.stringify({ features: ["TEST FEATURE MANUALLY PUT"], votes: {} }),
